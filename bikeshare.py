...
# A dictionary to hold the csv files
CITY_DATA = { 'chicago': 'chicago.csv',
              'new york': 'new_york_city.csv',
              'washington': 'washington.csv' }

# Lists that will be used to check the user's input and filter the DataFrame
...
# Function responsible for asking for the user's input and 
# filtering on just what the user wants to see
def get_filters():
    """
...
def load_data(city, month, day):
    ...
    # Creating columns to calculate the summary statistics
    ...
    # Filter first on the month, then the day of the week.
    # If a user doesn't enter "all", only the month they entered will be displayed.
    if month != 'all':
        month_index = months.index(month) + 1      
        df = df[df["month"] == month_index ]                

    # If a user doesn't enter "all", only the week day they entered will be displayed.
    if day != 'all':
        df = df[df["week_day"] == day.title() ]             

    return df

# Function responsible for computing the summary statistics related to time
def time_stats(df):
    ...
    print(f"Most common day of week: {most_common_dayofweek}")
...
def station_stats(df):
    ...
    print(f"Most commony used end station: {most_used_end_station}")

    # TO DO: display most frequent combination of start station and end station trip
    most_freq_stn_combination = df["start_end"].mode()[0]
    print(f"Most frequent combination of start and end station trip: {most_freq_stn_combination}")
...
def trip_duration_stats(df):
    ...
    print(f"Total travel time: {format_timespan(total_travel_time)}")   
    
    # TO DO: display mean travel time
    mean_time = df["Trip Duration"].mean()
    print(f"The mean travel time: {format_timespan(mean_time)}")
...
def user_stats(df):
    ...
    print(f"Counts of user types: {user_value_counts}")
